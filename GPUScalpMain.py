...
def scalp_bot_3000(link,pyplemail,pyplpw):
    ...
    travis.get(link)
    #Test Link
    #travis.get("https://www.bestbuy.com/site/logitech-m325-wireless-optical-mouse-works-with-chromebook-silver/2577677.p?skuId=2577677")
    Available = False

    while(Available != True):
        try:
            buy_item = travis.find_element_by_class_name("btn-disabled")
            print(travis.title)
            print("Out of Stock")
            travis.implicitly_wait(1)
            travis.refresh()
        except:
            buy_item = travis.find_element_by_class_name("btn-primary")
            print(travis.title)
        ...
        bestbuy_cart = travis.get("https://www.bestbuy.com/cart")
        #Wait for BestBuy Paypal Purchase Button to Load
        try:
            bestbuy_checkout = WebDriverWait(travis, 10).until(
            ...
            try:
                paypal_email_textbox = WebDriverWait(travis, 10).until(
                    EC.presence_of_element_located((By.ID, "email"))
                )
                paypal_email_textbox.send_keys(pyplemail)
                travis.find_element_by_id("btnNext").click()
                travis.implicitly_wait(1)
                paypal_password_textbox = travis.find_element_by_xpath('//*[@id="password"]')
                paypal_password_textbox.send_keys(pyplpw)
                login_paypal = travis.find_element_by_id("btnLogin")
                login_paypal.click()
                continue_paypal = travis.find_element_by_id("payment-submit-btn")
                continue_paypal.click()
                placeorder = travis.find_element_by_class_name("btn-primary")
                placeorder.click()
